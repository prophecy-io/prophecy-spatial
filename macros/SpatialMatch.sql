...
{% macro bigquery__SpatialMatch(
  ...
  {% set source_select = [] %}
  {% for col in source_columns %}
    {% set qc = prophecy_basics.quote_identifier(col) %}
    {% do source_select.append('source.' ~ qc) %}
  {% endfor %}

  {% set target_select = [] %}
  {% for col in target_columns %}
    {% set qc = prophecy_basics.quote_identifier(col) %}
    {% do target_select.append('target.' ~ qc ~ ' AS target_' ~ col) %}
  {% endfor %}
  ...
  SELECT
    {{ (source_select + target_select) | join(',\n    ') }}
  FROM {{ prophecy_basics.quote_identifier(source_relation) }} AS source
  CROSS JOIN {{ prophecy_basics.quote_identifier(target_relation) }} AS target
  WHERE
    {% if spatial_fn %}
      {{ spatial_fn }}(
        ST_GEOGFROMTEXT(source.{{ prophecy_basics.quote_identifier(source_col) }}),
        ST_GEOGFROMTEXT(target.{{ prophecy_basics.quote_identifier(target_col) }})
      )
    {% elif type == 'touches_or_intersects' %}
      ST_TOUCHES(
        ST_GEOGFROMTEXT(source.{{ prophecy_basics.quote_identifier(source_col) }}),
    ...
    {% elif type == 'envelope' %}
      ST_INTERSECTS(
        ST_ENVELOPE(ST_GEOGFROMTEXT(source.{{ prophecy_basics.quote_identifier(source_col) }})),
        ST_ENVELOPE(ST_GEOGFROMTEXT(target.{{ prophecy_basics.quote_identifier(target_col) }}))
      )
    {% else %}
      1=1 -- fallback if no known type
    {% endif %}
